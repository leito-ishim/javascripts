...
var let_cell;
var firstTimeForLet = false; // Временное препятствие
...
function move() {
    ...
    if (!isSnakeUnit(new_unit) && new_unit !== undefined && !isLetUnit(new_unit)) {
...
function haveFood(unit) {
    ...
    if (unit_classes.includes('food-unit')) {
        ...
        var currScore = document.getElementById('scores');
        currScore.innerHTML = 'Текущий счет: ' + score;
...
function createFood() {
    ...
    while (!foodCreated) { //пока еду не создали
        ...
        if (firstTimeForLet) {
            ...
            for (var i = 0; i < let_cell_classes.length-1; i++) {
                    classes += let_cell_classes[i] + ' ';
                    }
            let_cell.setAttribute('class', classes);
            }

        var let_x = Math.floor(Math.random() * FIELD_SIZE_X);
        var let_y = Math.floor(Math.random() * FIELD_SIZE_Y);

        let_cell = document.getElementsByClassName('cell-' + let_y + '-' + let_x)[0];
        var let_cell_classes = let_cell.getAttribute('class').split(' ');
